...
func (r *ReconcileAppService) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil && errors.IsNotFound(err) {
		...
		reqLogger.Info("Pod already exists", "Pod.Namespace", found.Namespace, "Pod.Name", found.Name)
	}

	// Check if this configmap already exists
	foundCm := &corev1.ConfigMap{}
	errCm := r.client.Get(context.TODO(), types.NamespacedName{Name: cm.Name, Namespace: cm.Namespace}, foundCm)
	if errCm != nil && errors.IsNotFound(errCm) {
		...
		errCm = r.client.Create(context.TODO(), cm)
		if errCm != nil {
			return reconcile.Result{}, errCm
		}
...
func newConfigMap(cr *appv1alpha1.AppService) *corev1.ConfigMap {
	labels := map[string]string{
		"app": cr.Name,
	}
